...
class AICConnection:
    ...
    async def send(self, msg: BaseServerMessage) -> None:
        await self._websocket.send_json({"type": msg.get_type(), **msg.model_dump(mode="json")})


async def connect(websocket: WebSocket) -> AICConnection:
...
def disconnect(connection: AICConnection) -> None:
...
async def send_message_to_all(msg: BaseServerMessage, source_connection_to_ommit: AICConnection | None = None) -> None:
